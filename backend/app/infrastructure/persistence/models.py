...
from app.domain.value_objects import OrganizationStatus, OrgMemberRole, TournamentFormat
...
class OrganizationModel(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, default=OrganizationStatus.pending.value
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    ...
    tournaments = relationship(
        "TournamentModel", back_populates="organization", cascade="all, delete-orphan"
    )
...
class OrganizationMemberModel(Base):
    ...
    role: Mapped[OrgMemberRole] = mapped_column(
        Enum(OrgMemberRole), nullable=False, default=OrgMemberRole.admin
    )
...
class TournamentModel(Base):
    ...
    pairing_strategy: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )  # random | by_ranking | similar_points_avoid_rematch
...
class MatchModel(Base):
    ...
    player3_id: Mapped[int] = mapped_column(
        ForeignKey("players.id", ondelete="CASCADE"), nullable=False
    )
    player4_id: Mapped[int] = mapped_column(
        ForeignKey("players.id", ondelete="CASCADE"), nullable=False
    )
