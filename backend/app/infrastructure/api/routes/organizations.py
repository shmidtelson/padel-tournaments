...
from app.infrastructure.api.dependencies import get_organization_service, require_current_user_id, require_superuser
...
async def create_organization(
    ...
    try:
        ...
        return OrganizationResponse(id=org.id, name=org.name, slug=org.slug, status=org.status, plan=getattr(org, "plan", "free"))
...
async def list_organizations(
    ...
    return [OrganizationResponse(id=o.id, name=o.name, slug=o.slug, status=o.status, plan=getattr(o, "plan", "free")) for o in orgs]
...
async def list_organization_members(
    ...
    try:
        ...
        return [
            OrganizationMemberResponse(id=m.id, user_id=m.user_id, organization_id=m.organization_id, role=m.role.value)
            for m in members
        ]
    except PermissionError as e:
        raise HTTPException(status_code=403, detail=str(e))
